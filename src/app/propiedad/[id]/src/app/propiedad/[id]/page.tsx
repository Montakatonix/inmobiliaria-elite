...
import { ArrowLeft, MapPin, Home, Phone, Mail } from 'lucide-react'
...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  ...
  return { title: p ? `${p.title} | Inmobiliaria Élite` : 'Propiedad | Inmobiliaria Élite' }
}
...
export default async function PropertyPage({ params }: { params: { id: string } }) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h1 className="heading-lg mb-4">Propiedad no encontrada</h1>
          <Link href="/comprar" className="btn-primary">Volver a propiedades</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <>
      <div className="pt-28 lg:pt-36 pb-20 bg-white">
        <div className="section-padding">
          <div className="container-elite">
            <Link href="/comprar" className="inline-flex items-center gap-2 text-sm text-brand-warm-gray hover:text-brand-gold transition-colors mb-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2">
                {/* Main image */}
                {property.images[0] && (
                  <div className="aspect-[16/10] rounded-sm overflow-hidden mb-3">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                    {property.images.slice(1).map((img: string, i: number) => (
                      <div key={i} className="aspect-[4/3] rounded-sm overflow-hidden">
                        <img src={img} alt={`Foto ${i + 2}`} className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" loading="lazy" />
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="lg:col-span-1">
                <div className="sticky top-32">
                  ...
                  <p className="flex items-center gap-1.5 text-brand-warm-gray text-sm mb-6">
                    <MapPin size={16} className="text-brand-gold" /> {property.location}
                  </p>
                  ...
                  {(property.rooms > 0 || property.bathrooms > 0 || property.size > 0) && (
                    <div className="flex gap-6 mb-8 pb-8 border-b border-gray-100">
                      ...
                      {property.bathrooms > 0 && <div><span className="block text-2xl font-bold text-brand-navy">{property.bathrooms}</span><span className="text-xs text-brand-warm-gray">Baños</span></div>}
                      {property.size > 0 && <div><span className="block text-2xl font-bold text-brand-navy">{property.size}</span><span className="text-xs text-brand-warm-gray">m²</span></div>}
                    </div>
                  )}
                  ...
                  <div className="bg-brand-light-bg p-6 rounded-sm mb-6">
                    ...
                    <div className="space-y-3">
                      <a href={`tel:${siteConfig.phone}`} className="btn-primary w-full justify-center">
                        <Phone size={16} /> Llamar ahora
                      </a>
                      <a href={`https://wa.me/34633077837?text=Hola, me interesa la propiedad: ${property.title} (Ref: ${property.id})`} target="_blank" rel="noopener noreferrer" className="btn-outline-gold w-full justify-center">
                        WhatsApp
                      </a>
                      <a href={`mailto:${siteConfig.email}?subject=Consulta propiedad ${property.id}&body=Hola, me interesa la propiedad: ${property.title}`} className="btn-secondary w-full justify-center">
                        <Mail size={16} /> Email
                      </a>
                    </div>
                  </div>
            ...
            <div className="mt-12 max-w-3xl">
              ...
              <div className="text-brand-slate leading-relaxed whitespace-pre-line">{property.description}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Contact */}
      <section className="bg-brand-light-bg py-16">
        <div className="section-padding">
          <div className="container-elite max-w-2xl">
            <h2 className="font-display text-2xl font-semibold text-brand-navy mb-6 text-center">Solicitar información</h2>
            <div className="bg-white p-8 rounded-sm shadow-sm border border-gray-100">
              <ContactForm variant="comprar" />
            </div>
          </div>
        </div>
      </section>
